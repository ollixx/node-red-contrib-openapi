...
const { joinPath, registerRoute, removeRoute } = require("./routing");
...
function registerMeta(RED, opts) {
  ...
  if (enable.json !== false) {
    registerRoute(RED, "get", jsonPath, [
      (req, res) => res.type("application/json").send(JSON.stringify(opts.raw)),
    ]);
    routes.push({ method: "get", path: jsonPath });
  }
  if (enable.yaml !== false) {
    registerRoute(RED, "get", yamlPath, [
      (req, res) => res.type("text/yaml").send(yaml.dump(opts.raw)),
    ]);
    routes.push({ method: "get", path: yamlPath });
  }
  if (enable.docs !== false) {
    const html = docsHtml(jsonPath, title);
    registerRoute(RED, "get", docsPath, [
      (req, res) => res.type("html").send(html),
    ]);
    routes.push({ method: "get", path: docsPath });
  }
  return { routes };
}
