...
function docsHtml(specUrl, title, assetBase) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8"/>
...
function registerMeta(RED, opts) {
  ...
  if (enable.docs !== false) {
    // Self-host the Swagger-UI assets from swagger-ui-dist instead of a CDN
    // (no runtime external dependency, works offline / under a strict CSP).
    let assetsDir = null;
    try {
      assetsDir = path.dirname(require.resolve("swagger-ui-dist"));
    } catch (e) {
      warnings.push("swagger-ui-dist not installed — /docs disabled");
    }
    if (assetsDir) {
      const html = docsHtml(jsonPath, title, docsPath);
      add(docsPath, (req, res) => res.type("html").send(html));
      add(joinPath(docsPath, "swagger-ui.css"), (req, res) =>
        res.sendFile(path.join(assetsDir, "swagger-ui.css"))
      );
      add(joinPath(docsPath, "swagger-ui-bundle.js"), (req, res) =>
        res.sendFile(path.join(assetsDir, "swagger-ui-bundle.js"))
      );
    }
  }
