...
/**
 * Whether a route for method+path is already registered on RED.httpNode.
 * @param {object} RED
 * @param {string} method http method
 * @param {string} path express path
 * @returns {boolean}
 */
function routeExists(RED, method, path) {
  const m = method.toLowerCase();
  const router = getRouter(RED);
  if (!router || !router.stack) return false;
  return router.stack.some(
    (l) => l.route && l.route.path === path && l.route.methods && l.route.methods[m]
  );
}

module.exports = { toExpressPath, joinPath, registerRoute, removeRoute, routeExists };
